...
class shortWebApp(webapp.webApp):
	...
	def process(self, parsedRequest):  #parsedRequest es la tupla (metodo,recurso, peticion) de la petición
		...
		if metodo == "GET":
			...
			else:								#si no hay recurso, mostramos el formulario CON LA LISTA DE URLS ACORTADAS
				code = "200 OK"
				body = "<html>Formulario<br><br>" + formulario + "<html><body><h1>Lista de URLs acortadas: </h1>" + csv.reader(self.__init__.myfile) + "</body></html>"
		elif metodo == "POST":
			url = peticion.split('\r\n\r\n',1)[1].split('=')[1]
			url = urllib.parse.unquote(url, encoding='utf-8', errors='replace')
			begin_url = url.split('://')[0] 
			if begin_url != "http" or begin_url != "https":
				url = "http://" + url
			...
			code = "200 OK"
			body = "<html>""URL real: "+ url +"<br><br>URL acortada: " + short_url + "</html>"
		return(code, body)
